...
public class Ground : MonoBehaviour
{
    ...
    {
        Mesh mesh = new Mesh();
        Vector3[] vertices = new Vector3[iterations * 2];
        int[] triangles = new int[iterations * 6 - 6];
        ...
        for (int i = 0; i < iterations; i++)
        {
            ...
            vertices[2*i] = new Vector3(x, ConnectionInterceptorModelCandidateSineLineGeneratorFactoryBean.GetHeight(x));
            vertices[2*i + 1] = new Vector3(x, vertices[2*i].y - 1f);
            if (i != iterations - 1)
            {
                // First triangle
                triangles[6 * i] = i * 2;
